(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function r(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(n){if(n.ep)return;n.ep=!0;const a=r(n);fetch(n.href,a)}})();function g(){const e=document.createElement("header"),t="/portfolio-uni/";e.className="header",e.innerHTML=`
  ...
  `;const r=e.querySelector("#mobileMenuBtn"),o=e.querySelector("#headerNav");return r?.addEventListener("click",()=>{o?.classList.toggle("active")}),e.querySelectorAll(".header-nav a").forEach(a=>{a.addEventListener("click",()=>{o?.classList.remove("active")})}),e}function f(e){const t=document.createElement("section"),r="/portfolio-uni/";return t.className="hero section",t.innerHTML=`
  ...
  `,t}function b(e){const t=document.createElement("section");return t.id="projects",t.className="section",t.innerHTML=`
    <div class="container">
  ...
  `,setTimeout(()=>w(),0),t}function y(e,t){return`
    <div class="card project-card fade-in">
      ...
      <div class="project-content">
        ...
        <p class="project-story">${m(e.story)}</p>
        ...
        <div class="project-lessons">
          <h4>Key Learnings</h4>
          <ul>
            ${e.lessonsLearned.map(r=>`<li>${m(r)}</li>`).join("")}
          </ul>
        </div>
      </div>
    </div>
  `}function d(e){const t={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"};return e.replace(/[&<>"']/g,r=>t[r])}function m(e){const t=/(https?:\/\/[^\s]+)/g;return e.replace(t,o=>`<a href="${o}" target="_blank" rel="noopener noreferrer" class="inline-link">${o}</a>`)}function w(){const e=document.getElementById("codeModal"),t=document.getElementById("codeModalTitle"),r=document.getElementById("codeModalCode")?.querySelector("code"),o=document.querySelector(".code-modal-close"),n=document.querySelectorAll(".code-preview");if(!e||!t||!r||!o)return;n.forEach(i=>{i.addEventListener("click",()=>{const l=i.getAttribute("data-project-title")||"",s=i.getAttribute("data-project-code")||"";t.textContent=l+" - Code Snippet",r.textContent=s,e.classList.add("active"),document.body.style.overflow="hidden"})});const a=()=>{e.classList.remove("active"),document.body.style.overflow=""};o.addEventListener("click",a),e.addEventListener("click",i=>{i.target===e&&a()}),document.addEventListener("keydown",i=>{i.key==="Escape"&&e.classList.contains("active")&&a()})}function C(e){const t=document.createElement("section"),r="/portfolio-uni/";return t.id="resume",t.className="section resume-section",t.innerHTML=`
  ...
  `,t}function S(e,t){return e.find(o=>o.platform===t)?.url||"#"}function k(e){const t=document.createElement("footer");return t.id="contact",t.className="footer",t.innerHTML=`
    <div class="container">
      <div class="footer-content">
        ...
        <div class="footer-section">
          ...
          <ul class="footer-links">
            ...
            <li><a href="${h(e.socials,"LinkedIn")}" target="_blank" rel="noopener noreferrer">LinkedIn Profile</a></li>
            <li><a href="${h(e.socials,"GitHub")}" target="_blank" rel="noopener noreferrer">GitHub Profile</a></li>
          </ul>
        </div>
...
}`,lessonsLearned:["<b>Serverless Microservices Architecture:</b> Cloudflare Workers for products, payments, orders + API gateway for routing and rate limiting","<b>Cross-platform App:</b> Single Flutter codebase supporting Android, iOS, and Web","<b>Tech Stack Integration:</b> Firebase + Cloudflare Workers + Stripe payments","<b>Real-time Inventory Management:</b> CRUD operations for products with immediate updates","<b>Order Management Panel:</b> Order status updates, tracking, admin oversight","This project is still live, so check it out! https://packwhisperer.com"]},{id:"crypto-idle-game",title:"Crypto Idle Mining Game with ApexCharts",videoUrl:"https://www.youtube.com/embed/TBvWNGk7U6Q",technologies:["HTML5","CSS","JavaScript","ApexCharts","LocalStorage"],story:"Built an idle mining game featuring <b>tier-based progression</b> and <b>real-time data visualization</b>. Implemented ApexCharts for displaying mining rates, currency accumulation, and tier progression. Demonstrated Software Development Life Cycle principles including requirements analysis, design, implementation, testing, and maintenance.",partialCode:`/**\r
...
}`,lessonsLearned:["<b>Cross-platform App:</b> Single Flutter codebase supporting Android, iOS, and Web","<b>Multi-view Architecture:</b> Created separate views (Main, Upgrades, Live Stats) with smooth view switching","<b>ApexCharts Integration:</b> Implemented three live-updating charts (Hash Power, Production Rate, Balance) using a third-party library","<b>Vanilla JavaScript:</b> No frameworks, pure DOM manipulation and event handling for lightweight performance"]},{id:"contoso-university",title:"Zenith Horizon University Management System",videoUrl:"https://www.youtube.com/embed/XvYvK8GpmXg",technologies:["ASP.NET Core 5 MVC","SQL Server","Entity Framework","Razor Views","Bootstrap"],story:"A fully functional <b>ASP.NET Core MVC 5 web application</b> that manages a university system with a modern, visually appealing interface. The project demonstrates a complete implementation of CRUD operations, relational database management, and responsive web design.",partialCode:`namespace ContosoUniversity.Controllers\r
{\r
...
}`,lessonsLearned:["<b>Entity Framework Relationships:</b> Complex but powerful","<b>TryUpdateModelAsync Security:</b> More secure than binding entire models to prevent overposting attacks","<b>User Experience:</b> Implemented Bootstrap for responsive and modern UI design","<b>Database Initialization:</b> Crucial for testing and development purposes","<b>MVC Architecture:</b> Comprehensive understanding of ASP.NET Core MVC architecture and routing mechanisms saved development time and improved maintainability"]}],R="Ema Guillen",M="Software Engineering Student",x="emajguillen@gmail.com",I="Building Quality Software | QA Enthusiast | Full-Stack Developer",B="Software Engineering student at BYU-Idaho. Passionate about software quality, testing automation, and building robust applications. Currently seeking Summer 2026 internship opportunities.",P=[{platform:"LinkedIn",url:"https://www.linkedin.com/in/ema-guillen/",icon:"linkedin"},{platform:"GitHub",url:"https://github.com/ejgdr",icon:"github"},{platform:"Email",url:"mailto:emajguillen@gmail.com",icon:"email"}],A="/resume/EmaGuillen_Resume.pdf",u={name:R,title:M,email:x,tagline:I,bio:B,socials:P,resumeUrl:A},v=document.querySelector("#app");v.innerHTML="";function H(){const e=g(),t=f(u),r=b(L),o=C(u),n=k(u);v.append(e,t,r,o,n),document.querySelectorAll('a[href^="#"]').forEach(l=>{l.addEventListener("click",function(s){s.preventDefault();const c=s.currentTarget.getAttribute("href");if(c&&c!=="#"){const p=document.querySelector(c);p&&p.scrollIntoView({behavior:"smooth",block:"start"})}})});const a={threshold:.1,rootMargin:"0px 0px -50px 0px"},i=new IntersectionObserver(l=>{l.forEach(s=>{s.isIntersecting&&(s.target.classList.add("fade-in"),i.unobserve(s.target))})},a);document.querySelectorAll(".card").forEach(l=>{i.observe(l)})}H();console.log("🚀 Portfolio loaded successfully!");console.log("Built with Vite + TypeScript");
